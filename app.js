...
async function completeRefund(id){const button=document.querySelector(`[data-complete-refund="${id}"]`);if(button){button.disabled=true;button.textContent="Completing…"}try{const {error}=await sb.from("refunds").update({status:"completed",completed_by:currentUser.id,completed_at:new Date().toISOString()}).eq("id",id);if(error)throw error;const item=button?.closest(".item");if(item)item.remove();const remaining=document.querySelectorAll("[data-complete-refund]").length;$("statRefunds").textContent=remaining;toast("Cash refund completed and removed from pending refunds.");await loadRefunds()}catch(e){if(button){button.disabled=false;button.textContent="Cash Given — Complete"}alert(friendlyError(e))}}
