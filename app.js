...
async function completeRefund(id){const button=document.querySelector(`[data-complete-refund="${id}"]`);if(button){button.disabled=true;button.textContent="Completing…"}try{const {data,error}=await sb.from("refunds").update({status:"completed",completed_by:currentUser.id,completed_at:new Date().toISOString()}).eq("id",id).eq("status","pending").select("id,status").maybeSingle();if(error)throw error;if(!data)throw new Error("The refund could not be completed. Please make sure the Supabase refund update policy is applied.");const item=button?.closest(".item");if(item)item.remove();$("statRefunds").textContent=document.querySelectorAll("[data-complete-refund]").length;await loadRefunds();toast("Cash refund completed and removed from pending refunds.")}catch(e){if(button){button.disabled=false;button.textContent="Cash Given — Complete"}console.error(e);alert(friendlyError(e))}}
